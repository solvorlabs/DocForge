  ...
  if (!result) {
    return (
      <div className="flex items-center gap-2 text-white/65">
        <Loader2 className="animate-spin w-4 h-4 rb-loading" /> Connecting...
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 w-full">
      <div className="flex items-center gap-2">
        <Loader2 className="animate-spin w-4 h-4 text-blue-400 rb-loading" />
      ...
      <div className="flex justify-between text-xs text-white/45">
        {STEPS.map((step, i) => (
          <span key={step} className={i <= stepIndex ? 'text-blue-300 font-medium' : ''}>
            {step}
          </span>
        ))}
      </div>
